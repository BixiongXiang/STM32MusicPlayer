...
void EXTI0_IRQHandler(void)
{
  ...
  //SoundReverse();// press wakeup down sound
...
void EXTI9_5_IRQHandler(void)
{
  if(EXTI_GetITStatus(EXTI_Line9) != RESET)
  {
    ...
    /*Generate sound*/
    //Sound();  //INTERRUPT PROGRAM CAN'T ENABLE TIME2, DELAY STILL WORK 
    
    /* Clear the EXTI line 9 pending bit */
    EXTI_ClearITPendingBit(EXTI_Line9);  // enable other interrupts
  }
}
...
{
  if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //判断更新事件
  {
  ...
  /* 反转PC.06电平 */
    ...
    GPIO_SetBits(GPIOB, GPIO_Pin_8);//increase square wave	 duty ratio
    GPIO_SetBits(GPIOB, GPIO_Pin_8);
    GPIO_SetBits(GPIOB, GPIO_Pin_8);
